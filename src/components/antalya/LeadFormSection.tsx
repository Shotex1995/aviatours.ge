...
export default function LeadFormSection() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const message = encodeURIComponent(
      `გამარჯობა! მინდა ანტალიის ტურის დაჯავშნა.\nსახელი: ${name || "—"}\nტელ: ${phone}`
    );
        ...
        {sent ? (
          ...
          <form onSubmit={handleSubmit} className="mx-auto max-w-md space-y-3">
            <input
              ...
              className="w-full rounded-xl border border-gray-300 bg-white px-5 py-3.5 text-gray-900 placeholder-gray-400 outline-none transition-all focus:border-orange-400 focus:ring-2 focus:ring-orange-400/20"
            />
            <div className="flex flex-col gap-3 sm:flex-row">
              <input
                ...
                className="flex-1 rounded-xl border border-gray-300 bg-white px-5 py-3.5 text-gray-900 placeholder-gray-400 outline-none transition-all focus:border-orange-400 focus:ring-2 focus:ring-orange-400/20"
              />
              <button type="submit" className="rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 px-6 py-3.5 font-bold text-white shadow-lg shadow-orange-500/25 transition-all hover:from-orange-600 hover:to-orange-700 active:scale-95">
                გაგზავნა
              </button>
            </div>
