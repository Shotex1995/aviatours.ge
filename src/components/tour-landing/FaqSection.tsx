...
import type { Faq } from "@/data/tour-landing.types";

export default function FaqSection({ faqs, subheading }: { faqs: Faq[]; subheading: string }) {
          ...
          {faqs.map((faq, i) => {
            ...
            return (
              <div key={i} className={`rounded-2xl border transition-all ${isOpen ? "border-orange-200 bg-white shadow-sm" : "border-gray-200 bg-gray-50"}`}>
                <button onClick={() => setOpenIndex(isOpen ? null : i)} className="flex w-full cursor-pointer items-center justify-between px-6 py-5 text-left font-semibold text-gray-900" aria-expanded={isOpen}>
                  {faq.q}
