...
import { calculateTotalPrice, georgianMonths } from "@/data/tour-landing.types";
import type { DepartureDate } from "@/data/tour-landing.types";
...
export default function PriceCalculator({ departureDates, budgetBasePrice, premiumBasePrice, tourName }: Props) {
  ...
  const basePrice = selected?.price ?? (tier === "budget" ? budgetBasePrice : premiumBasePrice);
  const total = calculateTotalPrice(basePrice, adults, children);
        ...
        <div className="space-y-6 rounded-2xl border border-gray-200 bg-gray-50 p-6">
          <div className="flex gap-2 rounded-xl bg-gray-200 p-1">
            {(["budget", "premium"] as const).map((t) => (
              <button key={t} onClick={() => { setTier(t); setDateIndex(0); }} className={`flex-1 rounded-lg py-2.5 text-sm font-bold transition-all ${tier === t ? "bg-white text-orange-600 shadow-sm" : "text-gray-500 hover:text-gray-700"}`}>
          ...
          <div>
            ...
            <select value={dateIndex} onChange={(e) => setDateIndex(Number(e.target.value))} className="w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-gray-900 outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-400/20">
              {filtered.map((d, i) => (
          ...
          <a href={`https://wa.me/995558302303?text=${whatsappMsg}`} target="_blank" rel="noopener noreferrer" className="block w-full rounded-xl bg-gradient-to-r from-orange-500 to-orange-600 py-4 text-center text-lg font-bold text-white shadow-lg shadow-orange-500/25 transition-all hover:from-orange-600 hover:to-orange-700 active:scale-95">
