import type { FinalCtaData } from "@/data/tour-landing.types";

export default function FinalCtaSection({ finalCta }: { finalCta: FinalCtaData }) {
  return (
    <section className="bg-gradient-to-r from-orange-500 to-orange-600 px-4 py-16 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl text-center">
        <h2 className="mb-4 text-3xl font-extrabold text-white sm:text-4xl">{finalCta.heading}</h2>
        <p className="mb-8 text-lg text-orange-100">{finalCta.subheading}</p>
        <div className="flex flex-col items-center justify-center gap-4 sm:flex-row">
          <a href={finalCta.bookingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-white px-10 py-4 text-lg font-bold text-orange-600 shadow-xl transition-all hover:bg-orange-50 active:scale-95">{finalCta.ctaText}</a>
