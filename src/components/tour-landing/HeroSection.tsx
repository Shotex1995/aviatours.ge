...
export default function HeroSection({ hero }: { hero: HeroData }) {
  ...
  return (
    <section className="relative h-[85vh] min-h-[600px]">
      <Image src={hero.backgroundImage} alt={hero.backgroundAlt} fill className="object-cover" sizes="100vw" priority />
      ...
      <div className="absolute inset-0 flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <h1 className="mb-4 text-5xl font-extrabold leading-tight text-white sm:text-6xl lg:text-7xl">{hero.title}</h1>
          <p className="mx-auto mb-3 max-w-2xl text-lg text-white/80 sm:text-xl">{hero.subtitle}</p>
          <p className="mb-4 text-3xl font-extrabold text-white sm:text-4xl">
            <span className="mr-2 text-xl font-normal text-white/50 line-through">{hero.oldPrice}</span>
            <span className="text-orange-400">{hero.currentPrice}</span>
          ...
          <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <a href={hero.bookingUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-orange-500 px-8 py-4 text-lg font-bold text-white shadow-2xl shadow-orange-500/30 transition-all hover:bg-orange-600 hover:shadow-orange-500/40 active:scale-95">
              დაჯავშნე ახლავე
            ...
            <a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[#25D366] px-8 py-4 text-lg font-bold text-white shadow-2xl shadow-green-500/30 transition-all hover:bg-[#1fba59] active:scale-95">
