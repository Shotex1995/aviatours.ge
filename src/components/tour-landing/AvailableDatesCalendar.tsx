...
import { georgianMonths, georgianDays } from "@/data/tour-landing.types";
...
export default function AvailableDatesCalendar({ departureDates }: { departureDates: DepartureDate[] }) {
  const now = new Date();
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div key={day} className={`relative flex h-10 items-center justify-center rounded-lg text-sm transition-colors ${dep ? "cursor-pointer bg-orange-100 font-bold text-orange-700 hover:bg-orange-200" : "text-gray-400"}`} title={dep ? `${dep.price.toLocaleString()}₾ — ${dep.seatsLeft} ადგილი` : undefined}>
                  {day}
                  {dep && <span className="absolute -bottom-0.5 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-orange-500" />}
                </div>
              );
            })}
          </div>
